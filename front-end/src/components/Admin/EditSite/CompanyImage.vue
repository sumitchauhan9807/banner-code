  ...
  <div>
    ...
    <div class="grid grid-cols-5 gap-4">
     ...
     <!-- <a href="#" @click.prevent="()=>{$refs.fileInput.click()}"><img style="max-height:500px"  class="h-auto max-w-full rounded-lg" :src="getUrl(image)" alt=""></a> -->
      <input @change.prevent="addCompanyImage" ref="fileInput" class=" mt-4 block w-1/2 text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 dark:text-gray-400 focus:outline-none dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400" id="file_input" type="file">
  </div>
</template>
<script>
/* eslint-disable */
   ...
   methods: {
    ...
    async addCompanyImage() {
      ...
      let formData = new FormData()
			 ...
			 formData.append("sitedata", 'companyImage');
      try{
				this.loading = true;
    ...
    async deleteCompanyImage(image) {
      try {
        this.loading = true;
        let {data} = await this.axios.delete(`admin/company-image/${image}`)
        this.loading = false;
