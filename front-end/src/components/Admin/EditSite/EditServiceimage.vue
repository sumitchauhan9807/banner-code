<template>
  <div>
    ...
    <div class="grid grid-cols-5 gap-4">
      ...
      <input @change.prevent="addServiceImage" ref="fileInput" class="  mt-4 block w-1/2 text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 dark:text-gray-400 focus:outline-none dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400" id="file_input" type="file">
  </div>
</template>
<script>
/* eslint-disable */
   ...
   methods: {
    ...
    async addServiceImage() {
      ...
      let formData = new FormData()
			 ...
			 formData.append("sitedata", 'serviceImage');
      try{
				this.loading = true;
    ...
    async deleteServiceImage(image) {
      try {
        this.loading = true;
        let {data} = await this.axios.delete(`admin/service-image/${image}`)
        this.loading = false;
